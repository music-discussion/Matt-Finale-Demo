...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'en-garde':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/en-garde/gardeDialogue'));
			case 'swordfight':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/swordfight/swordfightDialogue'));
			case 'combat':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/combat/combatDialogue'));
			case 'final-showdown':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/final-showdown/showdownDialogue'));	
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'sword-up':
			{
				//defaultCamZoom = 0.6; //(0.5, 0.5)
					defaultCamZoom = 0.5;
					curStage = 'lava-arena'; //lava-arena stage stuff
					var bg:FlxSprite = new FlxSprite(375, -450).loadGraphic(Paths.image('wiik5 stage/background_volacno', 'matt'));
					bg.antialiasing = true;
					...
					var gfStool:FlxSprite = new FlxSprite(-600, -1000).loadGraphic(Paths.image('wiik5 stage/gf_stool' , 'matt'));
					gfStool.antialiasing = true;
					gfStool.scrollFactor.set(0.9, 0.9);
					gfStool.active = false;
					gfStool.setGraphicSize(Std.int(gfStool.width * 1));
					gfStool.updateHitbox();
					add(gfStool);

					var backChar:FlxSprite = new FlxSprite(-600, -1000).loadGraphic(Paths.image('wiik5 stage/Bg_back' , 'matt'));
					backChar.antialiasing = true;
					backChar.scrollFactor.set(0.9, 0.9);
					backChar.active = false;
					backChar.setGraphicSize(Std.int(backChar.width * 1));
					backChar.updateHitbox();
					backChar.scale.set(1.5, 1.5);
					add(backChar);

					var playerStands:FlxSprite = new FlxSprite(300, -200).loadGraphic(Paths.image('wiik5 stage/volcano_stools' , 'matt'));
					playerStands.antialiasing = true;
					playerStands.scrollFactor.set(0.9, 0.9);
					playerStands.active = false;
					playerStands.setGraphicSize(Std.int(playerStands.width * 1));
					playerStands.updateHitbox();
					playerStands.scale.set(2.25, 2.25);
					add(playerStands);

					var frontChar:FlxSprite = new FlxSprite(-500, -850).loadGraphic(Paths.image('wiik5 stage/Bg_Front' , 'matt'));
					frontChar.antialiasing = true;
					frontChar.scrollFactor.set(0.9, 0.9);
					frontChar.active = false;
					frontChar.setGraphicSize(Std.int(frontChar.width * 1));
					frontChar.updateHitbox();
					frontChar.scale.set(1.1, 1.1);
					add(frontChar);
			}
			...
			case 'wiik5': 
			{
				defaultCamZoom = 0.8;
				curStage = 'wiik5';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('wiik5 stage/background_volacno', 'matt'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 1));
				bg.active = false;
		...
		switch (SONG.player2)
		{
			...
			dad.x += 100;
			gf.y += 300;
			case "chillmatt": //why does every matt character need a fucking offset.
			dad.y += 320;
			dad.x -= 50;
		}
		...
		switch (curStage)
		{
			...
			case 'swordarena': 
				boyfriend.x += 200;
			case 'lava-arena': //lava-arena stage char respositoing //camPos //(0.5, 0.5)
				boyfriend.x += 200;
				boyfriend.y -= 950;
				dad.y -= 950;
				dad.x += -100;
		...
		/*switch(curStage) //camPOS swordarena
		{
			case 'swordarena': // REPOSITIONING BF AND DAD YE
			/*	boyfriend.y = 400;
				boyfriend.x = -150;
				dad.y = 400;
				dad.x = 150;
	...
	switch (curBeat)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				#end
				if (SONG.song != 'Sword-Up')
					camFollow.setPosition(dad.getMidpoint().x + 150 + offsetX, dad.getMidpoint().y - 100 + offsetY);
				if (SONG.song.toLowerCase() == 'sword-up') //moving campos to center
					{
		...
		if (SONG.song.toLowerCase() == 'final-showdown') 
{
	switch (curBeat)
	{
		...
		/*switch(curStage) 
		{
			case 'swordarena': // MAKING CAMERA STATIC EVERY FRAME
				camPos.x = 0;
				camPos.y = 100;
		}*/
	}
